...
**Step 1 — Full frontend with mock data**
- Build all screens using the prototype as the design reference
- Screens: Dashboard (table + kanban), Detail Drawer, Login, Admin Panel
- All data hardcoded — no backend calls yet
- Do NOT proceed to Step 2 until the client has reviewed and approved the UI

**Step 2 — Client review and schema finalisation**
- Present Step 1 frontend to client for feedback
...
- Store email + create/update processo record; write activity log entry
...
- Detail drawer: add 3rd role (Resp. Compra) + attachments section with real data
- Real-time updates via polling (every 30s) or WebSocket
- Admin Panel: user management, status management, CSV/Excel import tool
