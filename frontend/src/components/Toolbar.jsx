...
export function Toolbar({ view, setView, search, setSearch, ownerFilter, setOwnerFilter, commFilter, setCommFilter, statusFilter, setStatusFilter, onFilterChange, currentUser }) {
  ...
  return (
    <div style={{ padding: "10px 24px", display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
      ...
      {/* Resp. Cotação */}
      <select value={ownerFilter} onChange={e => wrap(setOwnerFilter)(e.target.value)} style={SEL}>
        <option value="Todos">Resp. Cotação: Todos</option>
        {cotacaoOwners.map(o => <option key={o}>{o}</option>)}
      </select>

      {/* Resp. Comercial */}
      <select value={commFilter} onChange={e => wrap(setCommFilter)(e.target.value)} style={SEL}>
        <option value="Todos">Resp. Comercial: Todos</option>
        {comercialOwners.map(o => <option key={o}>{o}</option>)}
      </select>

      {/* Status */}
      <select value={statusFilter ?? "Todos"} onChange={e => { const v = e.target.value; wrap(setStatusFilter)(v === "Todos" ? null : v); }} style={SEL}>
